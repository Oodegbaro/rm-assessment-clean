...
// ---- Per-module progress (new 7-module system) ----
// Each of the 7 standalone module files reports its own completion here.
// Once all 7 are done for an email, this flips the SAME setTrainingComplete
// flag the assessment gate and admin dashboard already check -- nothing
// downstream needs to know the training system changed underneath it.
const TOTAL_MODULES = 7;

export async function setModuleComplete(email, moduleNum) {
  const key = `training-modules:${email}`;
  const existing = (await kv.get(key)) || [];
  const updated = [...new Set([...existing, moduleNum])].sort((a, b) => a - b);
  await kv.set(key, updated);

  const allDone = updated.length === TOTAL_MODULES;
  if (allDone) {
    await setTrainingComplete(email);
  }
  return { completedModules: updated, allDone };
}

export async function getModuleProgress(email) {
  const key = `training-modules:${email}`;
